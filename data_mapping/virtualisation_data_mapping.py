...
folder_path="/Users/sarthakkumar/IIA-Project/iia_project/LocalServiceProviders"
...
def find_mapped_attribute(db_name, synonyms, attribute_type):
    ...
    elif attribute_type in ["availability", "work_ex"]:
...
for db_file in db_files:
    ...
    if db_name not in existing_db_names:
        ...
        if cost_attr and availability_attr is not None:
            cursor.execute("INSERT INTO mapping (database_name, service_cost, availability, work_ex) VALUES (?, ?, ?, ?)",
                           (db_name, cost_attr, availability_attr, work_ex_attr))
            connection.commit()
